...
class SentimentAnalysisBoson():
    ...
    def sentiment_score_list(self,dataset):
        ...
        for sentence in seg_sentence: 
            ...
            for word in words:
                ...
                """
                poscount,negcount,poscount2,negcount2,poscount3,negcount3 = 0,0,0,0,0,0  # 
                if word in hp.dict.keys(): 
                    # print(word, hp.dict[word])
                    # print(words[min(i+1,len(words)-1)])
                    # print('-----')
                    if word in ['好','真','实在'] and words[min(i+1,len(words)-1)] in hp.dict  and words[min(i+1,len(words)-1)] != word:
                        continue
                    else:
                        poscount += hp.dict[word]
                        c = 0
                        for w in words[a:i]: # 扫描情感词前的程度词
                            if w in hp.mostdict:
                                poscount *= 4
                ...
                pos_count = poscount3
                # print(pos_count)
                count1.append(pos_count)      
            if words[-1] in ['!','！']:# 扫描感叹号前的情感词，发现后权值*2
                count1 = [j*2 for j in count1]
            ...
            for w_im in ['但是','但']:
                if w_im in words : # 扫描但是后面的情感词，发现后权值*5
                    ...
                    count1_head = count1[:ind]
                    count1_tail = count1[ind:]
                    count1_tail_new = [j*5 for j in count1_tail]
                    count1 = []
                    count1.extend(count1_head)
                    count1.extend(count1_tail_new)
                    break          
    ...
    def sentiment_score(self,s):
        ...
        if senti_score_list != []:
            ...
            for review in senti_score_list:
                score_array =  np.array(review)
                AvgPos = np.sum(score_array)
                # AvgNeg = np.sum(score_array[:,1])        
                # negatives.append(AvgNeg)
                positives.append(AvgPos)
            pos_score = np.mean(positives)
        else:
            pos_score = 0 #,neg_score=0,0
        return pos_score #,neg_score


if __name__ =='__main__':
    sa = SentimentAnalysis() #Boson()
    # text = '我妈说明儿不让出去玩'
    text = '怎么啦？但是特别难受吗'
    print(sa.sentiment_score(text))
